...
const JournalMain = ({ business, user, access }) => {
  ...
  const handleExport = async () => {
    try {
      const response = await api.post("fetch_journals", {
        business,
        user,
        parsed,
        searchQuery,
      ...
      if (response.status === "success") {
        ...
        toast.error(response.message || "Failed to export journals");
      }
    } catch (error) {
      if (error.response?.status === 401) {
        localStorage.removeItem("access");
        ...
        toast.error("Unexpected error while exporting journals");
      }
    }
  };
