...
import { faEdit, faTimesCircle, faUserGroup, faShareFromSquare } from "@fortawesome/free-solid-svg-icons";
import api from "../api";
...
const SupplierMain = ({ business, user, access }) => {
  ...
  const handleExport = async () => {
    try {
      const response = await api.post("fetch_suppliers", {
        business,
        user,
        searchQuery,
        page: 0,
      ...
      if (response.status === "success") {
        ...
        toast.error(response.message || "Failed to export suppliers");
      }
    } catch (error) {
      if (error.response?.status === 401) {
        localStorage.removeItem("access");
        ...
        toast.error("Unexpected error while exporting suppliers");
      }
    }
  };
