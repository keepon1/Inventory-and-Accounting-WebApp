...
const SalesPerformance = ({ business, user }) => {
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      result = result.filter(sale =>
        sale.customer_info__name.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const getTrendChartData = () => {
    ...
    filteredSales.forEach(sale => {
      ...
      if (!trendMap[period]) {
        trendMap[period] = { revenue: 0, orders: 0 };
      }
      trendMap[period].revenue += sale.gross_total;
  ...
  const getCustomerChartData = () => {
    const customerMap = {};
    filteredSales.forEach(sale => {
      if (!customerMap[sale.customer_info__name]) {
        customerMap[sale.customer_info__name] = 0;
      }
      customerMap[sale.customer_info__name] += sale.gross_total;
    });
  ...
  const renderChart = () => {
    switch (activeChart) {
      case 'revenue':
        return (
          <div>
          <h3 style={{ textAlign: 'center', marginBottom: '1rem' }}>Top 10 Sales</h3>
