...
const BrandMain = ({ business, user }) => {
        ...
        const fetchItems = async () => {
            try {
                ...
                if (response.status === 'error'){
                    toast.error(response.message || 'Error fetching brands');
                    return;
                }
    ...
    const addBrand = async () => {
        ...
        if (brands.some(brand => brand.name.toLowerCase() === detail.name.toLowerCase())) {
            toast.error('Brand name must be unique');
            return;
        }
        ...
        try {
            setLoading(true);
            const response = await api.post('add_brand', { business, detail: data, user });
            ...
            if (response.status === 'success') {
                ...
                const update = await api.post('fetch_brands', { business, user });

                if (update.status === 'error'){
                    toast.error(update.message || 'Error fetching brands');
                    return;
                }
                setBrands(update.data || []);
                setLoading(false);
            }else{
    ...
    const editBrand = async () => {
        ...
        try {
            setLoading(true);
            const response = await api.post('edit_brand', { business, detail: data, user });
    ...
    const deleteBrand = async () => {
        ...
        try {
            setLoading(true);
            const response = await api.post('delete_brand', { brand: editData.originalName});
                ...
                <div className="ivi_display_box1">
                    <div className="ivi_subboxes1">
                        <div className="ivi_holder_box1">
                            <input onChange={handleSearch} className='ivi_input' type="text" placeholder="Search brands..." />
                        </div>
                    </div>
                </div>
            </div>
                ...
                <table className="items-table">
                    ...
                    <tbody>
                        {filteredItems.map((brand, index) => (
                            <tr key={brand.name} id={`row-${index}`} className="table-row">
                                <td className="table-row">
                                    <button className="action-button" onClick={() => openEdit(brand)}>
            ...
            {showCreate && (
                <div className="modal-overlay">
                    <div className="modal" ref={overlayRef}>
                        ...
                        <div>
                            <button className="btn btn-outline" onClick={addBrand}>
                                Create Brand
                            </button>
                        </div>
                    </div>
                ...
                <div className="modal-overlay">
                    <div className="modal" ref={editOverlayRef}>
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            <button className="btn btn-outline" onClick={editBrand}>
                                Save Changes
                            </button>

                            <button className='btn btn-outline-red' style={{ marginLeft: '10px' }} onClick={deleteBrand}>
                                Delete Brand
                            </button>
                        </div>
